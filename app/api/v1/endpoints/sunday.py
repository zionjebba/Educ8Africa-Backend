...
TESTING_FIRST_DRAW_HOUR = 10
TESTING_FIRST_DRAW_MINUTE = 29
TESTING_SECOND_DRAW_HOUR = 10
TESTING_SECOND_DRAW_MINUTE = 33 
...
# Regular Sunday times: 10 AM and 2 PM
...
def get_next_sunday_draw_times():
    """Get the next Sunday's draw times."""
    now = datetime.now(DRAW_TIMEZONE)
    
    # Find next Sunday (or today if it's Sunday and before the draws)
    days_until_sunday = (6 - now.weekday()) % 7  # Sunday is 6
    
    # If it's Sunday, check if we've passed both draws
    if days_until_sunday == 0:
        ...
        if now < second_draw:
            ...
            # Passed both draws, move to next Sunday
            days_until_sunday = 7
    
    # Calculate next Sunday
    next_sunday = now + timedelta(days=days_until_sunday)
    return get_draw_times_for_date(next_sunday)
...
async def automatic_draw_trigger(db: AsyncSession, graph_client):
    ...
    # Only proceed if it's Sunday in production mode
    if not TESTING_MODE and now.weekday() != 6:
        print(f"   ⏭️  Not Sunday - skipping (weekday: {now.weekday()})")
        return None
    ...
    match_sunday = now.date()
    print(f"   📅 Match date: {match_sunday.strftime('%A, %B %d, %Y')}")
...
async def social_sunday_scheduler():
    ...
    while True:
        try:
            ...
            # In production, only run on Sundays
            if not TESTING_MODE and now.weekday() != 6:  # Sunday is 6
                if check_count % 4 == 0:  # Print every minute
                    print(f"⏰ Not Sunday - waiting...")
...
@router.get("/social-sunday/status")
async def get_draw_status(
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(aget_db)
):
    """Get the current status of Social Sunday match draws."""
    
    now = datetime.now(DRAW_TIMEZONE)
    is_sunday_today = now.weekday() == 6
    is_eligible_day = TESTING_MODE or is_sunday_today
    ...
    if is_eligible_day:
        ...
        if now < today_first_draw:
            ...
            first_draw_time, second_draw_time = get_next_sunday_draw_times()
...
async def create_match_pair_with_teams_meeting(
    ...
    video_call_link = None
    try:
        meeting_subject = f"Social Sunday Draw {draw_number}: {user1.first_name} & {user2.first_name}"
...
@router.get("/social-sunday/recent-matches")
async def get_recent_matches(
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(aget_db)
):
    """Get recent Social Sunday matches grouped by week with enhanced display info."""
    ...
    for week_start, week_matches in sorted(weeks.items(), reverse=True):
        ...
        sunday_date = week_start_date + timedelta(days=6)  # Sunday of that week
        
        # Determine if this is the current week
        is_current_week = week_start_date <= today <= week_end_date
        is_upcoming = sunday_date > today
        ...
        formatted_weeks.append({
            "week_label": f"{sunday_date.strftime('%B %d, %Y')}",
...
async def get_all_matches_for_modal(
    ...
    Accounts for TESTING_MODE to check today's date instead of just Sundays.
    """
    ...
    if TESTING_MODE:
        ...
        if current_weekday == 6:  # Is Sunday
            target_date = today
        else:
        ...
        print(f"[PRODUCTION MODE] Checking matches for Sunday: {target_date}")
...
@router.post("/social-sunday/matches/{match_id}/rate")
...
async def check_unseen_draws(
    ...
    if TESTING_MODE:
        ...
        # In production mode, check current or next Sunday
        current_weekday = today.weekday()
        if current_weekday == 6:  # Is Sunday
            target_date = today
        else:
            ...
            if days_until_sunday == 0:
                days_until_sunday = 7
            target_date = today + timedelta(days=days_until_sunday)
        print(f"[PRODUCTION MODE] Checking draws for Sunday: {target_date}")
