...
class User(Base, TimestampMixin):
    """Model representing a user in the IAxOS system."""
    ...
    profile_points_awarded = Column(Boolean, default=False, nullable=False)  # Track if user received points
    ...
    social_matches_as_user1 = relationship(
        "SocialMatch",
        back_populates="user1",
        foreign_keys="[SocialMatch.user1_id]"
    )
    social_matches_as_user2 = relationship(
        "SocialMatch",
        back_populates="user2",
        foreign_keys="[SocialMatch.user2_id]"
    )
